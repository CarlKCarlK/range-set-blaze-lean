...
private lemma lo_le_of_before {a b : NR} (h : a ≺ b) :
    a.val.lo ≤ b.val.lo := by
  have hlt : a.val.hi < b.val.lo := by
    have : a.val.hi < a.val.hi + 1 := lt_add_one _
    exact lt_trans this h
  exact le_trans a.property (le_of_lt hlt)

private lemma pairwise_lo_order {xs : List NR}
    (h : List.Pairwise (· ≺ ·) xs) :
    List.Pairwise (fun a b => a.val.lo ≤ b.val.lo) xs :=
  h.imp (fun _ _ hbefore => lo_le_of_before hbefore)

private lemma ordered_of_ok (s : RangeSetBlaze) :
    List.Pairwise (fun a b => a.val.lo ≤ b.val.lo) s.ranges :=
  pairwise_lo_order s.ok
...
private lemma coalesce_sets
    ...
    (horder : ∀ next ∈ pending, current.val.lo ≤ next.val.lo) :
  ...
  | cons next tail ih =>
      intro current
      have horder_head : current.val.lo ≤ next.val.lo := horder _ (by simp)
      have horder_tail :
          ∀ z ∈ tail, current.val.lo ≤ z.val.lo := by
        intro z hz
        exact horder z (by simp [hz])
      ...
      ·
        ...
        let merged :=
          mkNR current.val.lo (max current.val.hi next.val.hi)
            (by
              have hc : current.val.lo ≤ current.val.hi := current.property
              have hmax : current.val.hi ≤ max current.val.hi next.val.hi :=
                le_max_left _ _
              exact le_trans hc hmax)
        have hmergeEq :
            current.val.toSet ∪ next.val.toSet =
              merged.val.toSet := by
          have htouch' : ¬ current.val.hi + 1 < next.val.lo :=
            not_lt.mpr hle
          have := merge_step_sets current next horder_head htouch'
          simpa [merged, mkNR] using this
        have htail' :
            ∀ z ∈ tail, merged.val.lo ≤ z.val.lo := by
          intro z hz
          have := horder_tail z hz
          simpa [merged, mkNR] using this
        have hrec :=
          ih merged htail'
        have hrec' :
            listSet ((coalesce current (next :: tail)).fst ::
              (coalesce current (next :: tail)).snd) =
              merged.val.toSet ∪ listSet tail := by
          simpa [coalesce, htouch, merged] using hrec
        have hgoal :
            merged.val.toSet ∪ listSet tail =
              current.val.toSet ∪ listSet (next :: tail) := by
          calc
            merged.val.toSet ∪ listSet tail
                = (current.val.toSet ∪ next.val.toSet) ∪ listSet tail := by
                  simpa [hmergeEq]
        ...
        exact hrec'.trans hgoal
      ·
        have htouch' : ¬ next.val.lo ≤ current.val.hi + 1 :=
          of_decide_eq_false htouch
        simp [coalesce, htouch', listSet_cons, Set.union_left_comm,
